    ...
    public void receiveFromFollowees(Set<Candidate> candidates) {
    	...
    	for (int i = 0; i < nodes; ++i) {
    		if (followees[i]) {
    			...
    			// on any give round.
    			// TODO Is the first round is the most telling? Test this modification
    			Set<Transaction> txs  = followeeCandidates.get(i);
    			if (txs == null)
    				malicious[i] = true;
    			
    			// Nodes communicating only its own initial transactions. Don't consider
    			// a node malicious until some threshold of rounds
    			// 
    	...
    	// Above, nodes that send nothing are marked as malicious. Putting the
    	// call to add a candidate to valid here means that from the first 
    	// round, nodes that send nothing are excluded. This raises the score
    	// from 90/100 to 95/100.
    	// TODO: Test the effect of this when the 'roundsThreshold' code is
    	// not used at all.
    	for (Candidate c: candidates) {
    		if (followees[c.sender] && !malicious[c.sender])
    			validTxs.add(c.tx);	
